...
export class LocalStorageConversationRepository implements ConversationRepository {
  ...
  async findAll(): Promise<Conversation[]> {
    try {
      ...
      throw new StorageError(`Failed to retrieve conversations: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async findById(id: ConversationId): Promise<Conversation | null> {
    try {
      ...
      throw new StorageError(`Failed to retrieve conversation ${id.toString()}: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async save(conversation: Conversation): Promise<void> {
    try {
      ...
      const existingIndex = stored.findIndex(data => data.id === conversation.id.toString())
      ...
      throw new StorageError(`Failed to save conversation ${conversation.id.toString()}: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async delete(id: ConversationId): Promise<void> {
    try {
      ...
      throw new StorageError(`Failed to delete conversation ${id.toString()}: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async exists(id: ConversationId): Promise<boolean> {
    try {
      ...
      throw new StorageError(`Failed to check if conversation exists ${id.toString()}: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  private setStoredData(data: StoredConversation[]): void {
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'QuotaExceededError') {
        throw new StorageQuotaExceededError('localStorage quota exceeded. Please clear some data.')
      }
      throw error
    }
  }
  ...
  private serializeConversation(conversation: Conversation): StoredConversation {
    return {
      id: conversation.id.toString(),
      title: conversation.title,
      messages: conversation.getMessages().map(message => this.serializeMessage(message)),
  ...
  private deserializeConversation(data: StoredConversation): Conversation {
    try {
      const messages = data.messages.map(messageData => this.deserializeMessage(messageData))
      ...
      throw new Error(`Failed to deserialize conversation: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async clear(): Promise<void> {
    try {
      ...
      throw new StorageError(`Failed to clear all conversations: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async count(): Promise<number> {
    try {
      ...
      throw new StorageError(`Failed to count conversations: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
