...
describe('DeleteConversationUseCase', () => {
  ...
  describe('execute', () => {
    ...
    it('should handle repository exists errors', async () => {
      // Arrange
      vi.mocked(mockConversationRepository.exists).mockRejectedValue(new Error('Database error'))
    ...
    it('should handle repository delete errors', async () => {
      // Arrange
      vi.mocked(mockConversationRepository.exists).mockResolvedValue(true)
      vi.mocked(mockConversationRepository.delete).mockRejectedValue(new Error('Delete failed'))
  ...
  describe('error handling', () => {
    it('should log errors for debugging', async () => {
      ...
      vi.mocked(mockConversationRepository.exists).mockRejectedValue(new Error('Database connection failed'))
      ...
      // Assert
      expect(consoleSpy).toHaveBeenCalledWith('DeleteConversationUseCase error:', {
    ...
    it('should handle unknown error types', async () => {
      // Arrange
      vi.mocked(mockConversationRepository.exists).mockRejectedValue('String error')

      // Act & Assert
      await expect(useCase.execute({ conversationId: 'test-id' })).rejects.toThrow(
        'Failed to delete conversation. Please try again.'
      )
    })
    ...
    it('should return appropriate error message for not found errors', async () => {
      ...
      // Act & Assert
      await expect(useCase.execute({ conversationId: 'non-existent' })).rejects.toThrow(
        'The conversation could not be found or has already been deleted.'
      )
    })
